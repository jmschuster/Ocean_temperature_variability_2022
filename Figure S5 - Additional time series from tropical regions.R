...
MAP<-mp + geom_point(data = metadata2, 
  ...
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[1])] < center - 180 , 
                           metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[1])] + 360, 
                           metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[1])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[1])], label = 'a', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[2])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[2])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[2])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[2])], label = 'b', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[3])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[3])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[3])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[3])], label = 'c', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[4])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[4])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[4])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[4])], label = 'd', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[5])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[5])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[5])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[5])], label = 'e', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[6])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[6])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Tropical[6])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[6])], label = 'f', size =7,fontface='bold')
...
max_ylim = -99
min_ylim = 99
for (j in 1:length(additional_ts$Tropical)) {
  plot_idd = additional_ts$Tropical[j]
  ...
  if (length(data$plot_id) > 0) {
    ...
    if (j == 1) {
      ...
      index_start = which(data$measurement_date == "2012-11-08 01:00:00")
      index_end = index_start + 365*24*(60/freq)
      data = data[index_start:index_end,]
    }
    ...
    assign(paste0("plot_",j), ggplot(data = get(paste0("data",j)), aes(x = new_date, y = temp)) + 
             ...
             geom_line(color='#FC440F') + theme_classic() + 
             labs(title = paste0(metadata$loc_name[which(metadata$plot_id == additional_ts$Tropical[j])],
                                 " (", ifelse(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])],
                                                     1,2) == "'-", 
                                              paste0(as.character(format(round(as.double(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])],
                                                                                      3,nchar(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])]))), 2), nsmall = 2)), "°S"),
                                              ifelse(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])],
                                                            1,1) == "-", 
                                                     paste0(as.character(format(round(as.double(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])],
                                                                                             2,nchar(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])]))), 2), nsmall = 2)), "°S"),
                                                     paste0(as.character(format(round(as.double(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Tropical[j])], 2)), nsmall = 2)), "°N"))),
                                 ", ", freq, " mins)"
                                 )) + 
...
for (j in 1:length(additional_ts$Tropical)) {
  ...
  if (j == 1) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "380")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "380")[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.5
    y3 = 0.83
  } else if (j == 2) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "185")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "185")[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.5
    y3 = 0.82
  } else if (j == 3) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "406")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "406")[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.46
    y3 = 0.69
  } else if (j == 4) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "98")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "98")[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.18
    y3 = 0.72
  } else if (j == 5) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "428")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "428")[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.5
    y3 = 0.77
  } else if (j == 6) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "395")[15]], 2), nsmall = 2) # 15 because you start the time series in 2012
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "395")[15]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.38
    y3 = 0.79
  }
...
ggsave(filename="FigS5.png",height=9, width=12.5, plot=grid.arrange(plot_1, plot_2,plot_3,
                                                                            arrangeGrob(blankPlot,MAP,LatRange,widths=c(0.08,0.8,0.12)),
