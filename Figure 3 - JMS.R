...
### PLOT 1
plot1 = ggplot(data=temp_range1, aes(x=window, y=mediann, fill=climate_classification)) + 
  geom_rect(xmin = 4.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = 'lightgrey', alpha = 0.5) + 
  geom_boxplot(outlier.shape=NA)+geom_point(data=subset(bayesian_coeffs,X<=15),size=2,position=position_dodge(width=0.75),show_guide = FALSE) + 
  geom_errorbar(data=subset(bayesian_coeffs,X<=15),
...
plot1 = plot1 + coord_cartesian(ylim = c(0,25.00))
plot1 
...
### PLOT 2
plot2 = ggplot(data=temp_range2, aes(x=window, y=mediann, fill=climate_classification))+ 
  geom_rect(xmin = 4.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = 'lightgrey', alpha = 0.5) + 
  geom_boxplot(outlier.shape=NA) +geom_point(data=subset(bayesian_coeffs_br_diff,X<=15),size=2,position=position_dodge(width=0.75),show_guide = FALSE) + 
  geom_errorbar(data=subset(bayesian_coeffs_br_diff,X<=15),
                aes(ymin=lower.HPD, ymax=upper.HPD),position=position_dodge(width=0.75),width=0.1)+ 
  labs(y = "Median",
       fill = "Climate Classification") + theme_bw() + theme(plot.title = element_text(hjust = 0.5,
                                                                                         size=15),
...
plot2 = plot2 + coord_cartesian(ylim = c(0,0.0015))
plot2
...
plot3 = ggplot(data=temp_range3, aes(x=window, y=ninetieth_perc, fill=climate_classification))+ 
  geom_rect(xmin = 4.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = 'lightgrey', alpha = 0.5) + 
  geom_boxplot(outlier.shape=NA)+geom_point(data=subset(bayesian_coeffs90),size=2,position=position_dodge(width=0.75),show_guide = FALSE) + 
  geom_errorbar(data=subset(bayesian_coeffs90),
                aes(ymin=lower.HPD, ymax=upper.HPD),position=position_dodge(width=0.75),width=0.1) + 
...
plot3 = plot3 + coord_cartesian(ylim = c(0,25))
plot3
...
plot4 = ggplot(data=temp_range4, aes(x=window, y=ninetieth_perc, fill=climate_classification))+ 
  geom_rect(xmin = 4.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = 'lightgrey', alpha = 0.5) + 
  geom_boxplot(outlier.shape=NA)+geom_point(data=bayesian_coeffs_br_diff90,size=2,position=position_dodge(width=0.75),show_guide = FALSE) + 
  geom_errorbar(data=bayesian_coeffs_br_diff90,
                aes(ymin=lower.HPD, ymax=upper.HPD),position=position_dodge(width=0.75),width=0.1)  + 
...
plot4 = plot4 + coord_cartesian(ylim = c(0,0.0015))
plot4

all_plots = grid.arrange(plot1, plot2,
                         plot3, plot4, ncol = 2)

pdf(width = 12, useDingbats=TRUE,height = 7.55, bg="white", file="Fig_3_JMS") 
all_plots = grid.arrange(plot1, plot2,
                         plot3, plot4, ncol = 2,
...
# ggsave plot
ggsave(filename="Fig3_JMS.png",height=5, width=9, units="in", plot=grid.arrange(plot1, plot2,
                                                        plot3, plot4, ncol = 2, 
                                                        widths = c(2.7, 2.7), heights = c(2.5, 2.5)), device="png")
