...
MAP<-mp + geom_point(data = metadata2, 
  ...
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[1])] < center - 180 , 
                           metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[1])] + 360, 
                           metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[1])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[1])], label = 'a', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[2])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[2])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[2])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[2])], label = 'b', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[3])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[3])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[3])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[3])], label = 'c', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[4])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[4])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[4])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[4])], label = 'd', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[5])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[5])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[5])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[5])], label = 'e', size =7,fontface='bold') +
  annotate('text',
           x=ifelse(metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[6])] < center - 180 , 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[6])] + 360, 
                    metadata$long_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[6])]), 
           y = metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[6])], label = 'f', size =7,fontface='bold')
...
max_ylim = -99
min_ylim = 99
for (j in 1:length(additional_ts$Subtropical)) {
  plot_idd = additional_ts$Subtropical[j]
  ...
  if (length(data$plot_id) > 0) {
    ...
    if (j == 1) {
      ...
      index_start = which(substr(data$measurement_date,1,10) == "2014-03-22")[1]
      index_end = index_start + 365*24*(60/freq)
      data = data[index_start:index_end,]
    } else if (j == 3) {
      index_start = 1
      ...
      index_start = which(data$measurement_date == "2018-03-20 01:00:00")
      index_end = index_start + 365*24*(60/freq)
      data = data[index_start:index_end,]
    } else if (j == 6) {
      index_start = 1 
    ...
    assign(paste0("plot_",j), ggplot(data = get(paste0("data",j)), aes(x = new_date, y = temp)) + 
             ...
             geom_line(color='#E8C547') + theme_classic() + 
             labs(title = paste0(metadata$loc_name[which(metadata$plot_id == additional_ts$Subtropical[j])],
                                 " (", ifelse(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])],
                                                     1,2) == "'-", 
                                              paste0(as.character(format(round(as.double(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])],
                                                                                      3,nchar(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])]))), 2), nsmall = 2)), "°S"),
                                              ifelse(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])],
                                                            1,1) == "-", 
                                                     paste0(as.character(format(round(as.double(substr(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])],
                                                                                             2,nchar(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])]))), 2), nsmall = 2)), "°S"),
                                                     paste0(as.character(format(round(as.double(metadata$lat_in_degrees[which(metadata$plot_id == additional_ts$Subtropical[j])], 2)), nsmall = 2)), "°N"))),
                                 ", ", freq, " mins)"
                                 )) + 
             ...
             theme(plot.title = element_text(hjust = 0.5),
                   ...
                   text = element_text(size=11)) + ylab("Temperature (°C)") + 
...
for (j in 1:length(additional_ts$Subtropical)) {
  ...
  if (j == 1) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == "29")[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == "29")[1]], 2), nsmall = 2)
    y1 = 0.92
    y2 = 0.16
    y3 = 0.6
  } else if (j == 2) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[4]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[4]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.37
    y3 = 0.84
  } else if (j == 3) {
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[1]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[1]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.49
    y3 = 0.86
  } else if (j == 4) {
    ...
    daily_median = format(round(temp_range$daily_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[2]], 2), nsmall = 2)
    monthly_median = format(round(temp_range$monthly_median[which(temp_range$plot_id == additional_ts$Subtropical[j])[2]], 2), nsmall = 2)
    y1 = 0.1
    y2 = 0.5
    y3 = 0.81
  } else if (j == 6) {
...
ggsave(filename="FigS6.png",height=9, width=12.5, plot=grid.arrange(plot_1, plot_2,plot_3,
                                                                            arrangeGrob(blankPlot,MAP,LatRange,widths=c(0.08,0.8,0.12)),
